...
--      V19 previously failed because some tables created in V1-V14 (which were
--      skipped via baseline-version=14) do not exist in production.
--      This revision uses DO blocks to safely skip inserts for missing tables.

-- ─────────────────────────────────────────────────────────────────────────────
-- HERO SLIDES  (replace all with Unsplash images — table confirmed to exist)
-- ─────────────────────────────────────────────────────────────────────────────
...
VALUES
    ('camp-edu-001', 'Build School in Rural Village', 'build-school-rural-village',
     ...
     'Help us build a fully equipped primary school with 6 classrooms, library, and playground in a rural village.',
     500000, 125000, 'usd', 'cat-001', 'https://images.unsplash.com/photo-1688789029020-6c5b8b22305a?w=800',
    ...
    ('camp-edu-002', 'Scholarship Fund for Girls', 'scholarship-fund-girls',
     ...
     75000, 28000, 'usd', 'cat-001', 'https://images.unsplash.com/flagged/photo-1574097656146-0b43b7660cb6?w=800',
    ...
    ('camp-food-001', 'Daily Meals for Children', 'daily-meals-children',
     ...
     'Fight childhood hunger by providing breakfast and lunch to 500 children from slum areas.',
    ...
    ('camp-food-003', 'Food Bank', 'food-bank-families',
     ...
     60000, 22000, 'usd', 'cat-002', 'https://images.unsplash.com/photo-1599059813005-11265ba4b4ce?w=800',
    ...
    ('camp-agri-001', 'Irrigation for Farmers', 'irrigation-system-farmers',
     ...
     80000, 35000, 'usd', 'cat-003', 'https://images.unsplash.com/photo-1743742566156-f1745850281a?w=800',
    ...
    ('camp-health-003', 'Maternity Care', 'maternity-health-program',
     ...
     70000, 25000, 'usd', 'cat-004', 'https://images.unsplash.com/photo-1457342813143-a1ae27448a82?w=800',
    ...
    ('camp-skill-002', 'Tailoring Training', 'tailoring-training-women',
     ...
     45000, 15000, 'usd', 'cat-005', 'https://plus.unsplash.com/premium_photo-1664195857591-968c2a9f2c03?w=800',
    ...
    ('camp-women-002', 'Self Defense Training', 'self-defense-training',
     ...
     25000, 8000, 'usd', 'cat-006', 'https://plus.unsplash.com/premium_photo-1667941272772-70243b7b4f63?w=800',
    ...
    ('camp-water-001', 'Village Water Wells', 'village-water-wells',
     ...
     120000, 55000, 'usd', 'cat-008', 'https://images.unsplash.com/photo-1771619643236-5f710672c3be?w=800',
    ...
    ('camp-water-002', 'Water Purifiers', 'water-purification-schools',
     ...
     50000, 18000, 'usd', 'cat-008', 'https://images.unsplash.com/photo-1629805019342-9acdfabfc990?w=800',
     'West Bengal', 3000, true, false, false, NOW(), NOW())
ON CONFLICT (id) DO NOTHING;

-- ─────────────────────────────────────────────────────────────────────────────
-- OPTIONAL TABLES  (may not exist — skip gracefully if missing)
-- ─────────────────────────────────────────────────────────────────────────────
DO $$
BEGIN
    ...
    -- homepage_stats
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'homepage_stats') THEN
        INSERT INTO homepage_stats (id, stat_label, stat_value, icon, active, display_order, created_at, updated_at) VALUES
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'testimonials') THEN
        INSERT INTO testimonials (id, author_name, author_title, testimonial_text, active, display_order, created_at, updated_at) VALUES
            ('test-001', 'Priya Sharma', 'Scholarship Recipient',
             'Thanks to the scholarship fund, I completed my education and now work as a teacher.',
             true, 1, NOW(), NOW()),
            ('test-002', 'Rajesh Kumar', 'Farmer, Punjab',
             'The drip irrigation system increased my crop yield by 45%. My family''s income has doubled.',
             true, 2, NOW(), NOW()),
            ('test-003', 'Anita Desai', 'Women Entrepreneur',
             'The microloan and training helped me start my tailoring business. I now employ 3 other women.',
             true, 3, NOW(), NOW())
        ON CONFLICT (id) DO NOTHING;
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'site_config') THEN
        INSERT INTO site_config (id, config_key, config_value, description, created_at, updated_at) VALUES
